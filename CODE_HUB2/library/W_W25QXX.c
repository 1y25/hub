...
void WQ_RamRead(uint32_t addr,int8_t front)
{
	...
	uint32_t t;
	t = 2000000;
	while(DMA_GetFlagStatus(DMA1_FLAG_TC3)!=SET && t--);
	t = 2000000;
	while(DMA_GetFlagStatus(DMA1_FLAG_TC2)!=SET && t--);
	WQ_ReadStop();
	DMA_Cmd(DMA1_Channel3,DISABLE);
	DMA_Cmd(DMA1_Channel2,DISABLE);
}
...
uint8_t WQ_Swap(uint8_t data)
{
	...
	uint32_t t;
	t = 200000;
	while(SPI_I2S_GetFlagStatus(SPI1,SPI_I2S_FLAG_TXE)!=SET) { if(--t==0) break; }
	t = 200000;
	while(SPI_I2S_GetFlagStatus(SPI1,SPI_I2S_FLAG_BSY)==SET) { if(--t==0) break; }
	t = 200000;
	while(SPI_I2S_GetFlagStatus(SPI1,SPI_I2S_FLAG_RXNE)!=SET) { if(--t==0) break; }
	t = 200000;
	while(SPI_I2S_GetFlagStatus(SPI1,SPI_I2S_FLAG_BSY)==SET) { if(--t==0) break; }
	return SPI_I2S_ReceiveData(SPI1);
}
/**@brief  等待操作完成(带超时保护, 防止卡死)
  */
